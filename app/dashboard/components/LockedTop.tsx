export default function LockedTop({ items }: { items: any[] }) {
  if (!items || items.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">
          Top Simple Earn Locked
        </h2>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700">
              ...
              <th className="py-3 px-4 text-gray-700 dark:text-gray-300">Duracion</th>
              <th className="py-3 px-4 text-gray-700 dark:text-gray-300">Minimo</th>
            ...
            {items.map((item, index) => (
              <tr key={index} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="py-3 px-4 text-gray-600 dark:text-gray-400">
                  {item.duration ? `${item.duration} dias` : "N/A"}
                </td>
                <td className="py-3 px-4 text-gray-600 dark:text-gray-400">
                  {item.min || "N/A"}
                </td>
